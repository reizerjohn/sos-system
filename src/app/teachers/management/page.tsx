...
import UserTable from '@app/components/teacher-management/teacher-table';
...
const UsersPage = () => {
  ...
  const { pageCount, setPageCount, hasNextButton, search, setSearch } = useUserContext();
  ...
  return (
    <div className='m-10'>
      <div className='flex flex-row justify-between items-center mb-5'>
        <h1 className='text-3xl'>Teachers Management</h1>
        <div className='flex space-x-2 w-1/2'>
          ...
          <div className='flex flex-row space-x-2'>
            <Button
              ...
              type='submit'
              radiusSize='lg'
              onClick={() => router.push(TEACHERS_PAGE)}
            >
              Add Teacher
            </Button>
          </div>
      ...
      <div className='mt-4 flex justify-between'>
        <button disabled={pageCount === 1 || isLoading} onClick={() => handlePageButton('previous')} className={pageButtonStyle}>
          <Icon path={mdiArrowLeft} size={0.8} />
          &nbsp;Previous
        </button>
        <button disabled={!hasNextButton || isLoading} onClick={() => handlePageButton('next')} className={pageButtonStyle}>
          Next&nbsp;
