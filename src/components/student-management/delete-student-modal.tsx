...
const DeleteStudentModal = ({ isOpen, onRequestClose, closeModal, student }: Props) => {
  const { reset } = useStudentContext();

  const handleSubmit = async () => {
    const body = { id: student.id, type: 'student' };

    try {
      const res = await clientApi.delete('users/students', { params: body });
  ...
  return (
    <Modal isOpen={isOpen} onRequestClose={onRequestClose} style={customStyles} contentLabel='Delete Student Modal'>
      <div className='w-full h-full flex items-start space-x-4'>
        ...
        <div className='space-y-3'>
          <div className='space-y-1 pl-2'>
            <p className='label-lg text-neutral-800'>Are you sure you want to delete this student?</p>
            <p className='body-2 text-neutral-600'>
              You are about to delete <span className='font-semibold'>{`${student.name}`}</span>? This cannot be undone.
            </p>
          </div>
